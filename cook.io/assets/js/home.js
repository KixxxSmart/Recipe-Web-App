...
$searchBtn.addEventListener("click", function () {
    if ($searchField.value) {
        window.location = `/recipes.html?q=${$searchField.value}`; // Use backticks (`) for template literals
    }
});
...
addEventOnElements($tabBtns, "click", function () {
    ...
    $lastActiveTabBtn.setAttribute("aria-selected", false); // Fix the typo here
...
addEventOnElements($tabBtns, "keydown", function(e) {
    ...
    if (e.key === "ArrowRight" && $nextElement) {
        ...
        this.setAttribute("tabindex", -1);
        $lastActiveTabBtn.setAttribute("tabindex", 0);  // Fix the typo here
    }
});
...
const addTabContent = ($currentTabBtn, $currentTabPanel) => {
    ...
    fetchData([['mealType', $currentTabBtn.textContent.trim().toLowerCase()], ...cardQueries], function(data){
        ...
        for (let i = 0; i < 12; i++) {
            ...
            $card.innerHTML = `
                <figure class="card-media img-holder">
                    <img src="${image}" width="195" height="195" loading="lazy" alt="${title}" class="image-cover">
                        </figure>
        ...
        $currentTabPanel.innerHTML += `
        <a href="./recipes.html?mealType=${$currentTabBtn.textContent.trim().toLowerCase()}" class="btn btn-seconday label-large has-state">Show more</a>
        `;
